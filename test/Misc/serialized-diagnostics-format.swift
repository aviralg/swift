...
// '=sarif' is accepted and selects the SARIF path, so no '.dia' is written. The
// '.sarif' itself arrives with the consumer in a later PR.
// RUN: cp %s %t/sarif.swift
// RUN: cd %t && %target-swift-frontend -typecheck -serialize-diagnostics=sarif sarif.swift
// RUN: test ! -f %t/sarif.dia
